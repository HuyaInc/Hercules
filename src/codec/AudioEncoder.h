...
namespace hercules
{
    ...
    class AudioEncoder : public OneCycleThread, public Property
    {
    public:
        AudioEncoder();
        ~AudioEncoder();
        ...
        int init(const std::string &name,
            Queue<MediaFrame> &in_queue, Queue<MediaPacket> &out_queue);

        bool popMediaFrame(MediaFrame &);
        void pushMediaFrame(MediaFrame &);
        void pushMediaPacket(MediaPacket &);
        bool popMediaPacket(MediaPacket &);

        void setCodec(int channels, int sampleRate,
            int kbps, const std::string &codec);
        void setBitrate(int bitrate);
        void setLowLatency(bool lowLatency) { m_lowLatency = lowLatency; }

        virtual std::string traceInfo() { return ""; }
